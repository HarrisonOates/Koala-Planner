...
use super::task_structs::{Method, Task};
...
impl HTN {
    ...
    pub fn count_tasks_with_frequency(&self) -> HashMap<u32, u32> {
        let task_ids: Vec<_> = self.mappings.iter().map(|(_, v)| *v).collect();
    ...
    pub fn relabel_nodes(tn: &HTN, start_index: u32) -> HTN {
        let new_ids: HashMap<u32, u32> = tn
            .network
            .nodes
            .iter()
            .cloned()
        ...
        let new_mappings: HashMap<u32, u32> = tn
            .mappings
            .iter()
            .map(|(k, v)| match new_ids.get(k) {
                Some(new_id) => (*new_id, *v),
                None => {
        ...
        HTN {
            network: subgraph,
            domain: tn.domain.clone(),
            mappings: new_mappings,
        }
    }

    pub fn get_all_task_mappings(&self) -> Vec<u32> {
        self.mappings.iter().map(|(_node, task)| *task).collect()
    }
    ...
    pub fn separate_tasks(&self, tasks: &BTreeSet<u32>) -> (BTreeSet<u32>, BTreeSet<u32>) {
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> Result<(), fmt::Error> {
        ...
        for node in self.get_nodes().iter() {
            writeln!(
                ...
                "\t{} [label={}];",
                *node,
                self.get_task(*node).borrow().get_name()
            );
        }
...
mod tests {
    ...
    fn decomposition_tasks() -> (Task, Task, Task, Task, Task) {
        let empty = HashSet::new();
        let t1 = Task::Primitive(PrimitiveAction::new(
    ...
    fn decomposition_test() {
        ...
        let t3_method = Method::new(
            ...
            HTN::new(
                ...
                domain.clone(),
                HashMap::from([
                    (1, domain.get_id(&t5.get_name())),
                    (2, domain.get_id(&t6.get_name())),
                    (3, domain.get_id(&t7.get_name())),
                    (4, domain.get_id(&t8.get_name())),
        ...
        if let Task::Compound(CompoundTask { name: _, methods }) = &*c_task.borrow() {
    ...
    pub fn isomorphism_test() {
        ...
        let alpha = HashMap::from([(1, 1), (2, 2), (3, 3), (4, 4)]);
        let htn1 = HTN::new(nodes1, orderings1, domain.clone(), alpha);
    ...
    pub fn is_primitive_test() {
        ...
        let orderings1: Vec<(u32, u32)> = Vec::from([(1, 3), (2, 3), (3, 4)]);
        let alpha = HashMap::from([(1, 0), (2, 1), (3, 2), (4, 3)]);
        let htn = HTN::new(nodes1, orderings1, domain.clone(), alpha);
    ...
    pub fn last_action_test() {
        ...
        let alpha = HashMap::from([(1, 0), (2, 1), (4, 3)]);
        let htn = HTN::new(nodes1, orderings1, domain.clone(), alpha);
        let new_htn = htn.apply_action(2);
        let new_htn_2 = new_htn.apply_action(1);
    ...
    pub fn is_empty() {
        ...
        let alpha = HashMap::from([(1, 1), (2, 2), (4, 4)]);
        let htn = HTN::new(nodes, orderings, domain.clone(), alpha);
        assert_eq!(htn.is_empty(), false);

        let empty_htn = HTN::new(BTreeSet::new(), Vec::new(), domain.clone(), HashMap::new());
    ...
    pub fn relabel_test() {
        ...
        assert_eq!(
            new_tn.get_task(2).borrow().get_name(),
            format!("ObtainPermit")
        );
        assert_eq!(
            new_tn.get_task(3).borrow().get_name(),
            format!("HireBuilder")
        );
        assert_eq!(new_tn.get_task(4).borrow().get_name(), format!("Construct"));
        assert_eq!(
            new_tn.get_task(5).borrow().get_name(),
            format!("PayBuilder")
        );
    }
    ...
    pub fn collapse_tn_test() {
        ...
        assert_eq!(
            problem.init_tn.get_unconstrained_tasks(),
            BTreeSet::from([1])
        );
    }
    ...
    pub fn recursive_decomposition_test() {
        ...
        let domain = Rc::new(DomainTasks::new(vec![t]));
