...
impl TDG {
    pub fn new(tn: &HTN) -> TDG {
        ...
        let root = *tn.get_unconstrained_tasks().iter().next().unwrap();
        ...
        while !working_set.is_empty() {
            ...
            match &*domain.get_task(task_id).borrow() {
                Task::Compound(compound) => match task_vertices.get(&task_id) {
                    Some(_) => {}
                    None => {
                        ...
                        for (i, method) in compound.methods.iter().enumerate() {
                            ...
                            let subtasks: BTreeSet<u32> = subtasks
                                .iter()
                                .map(|x| domain.get_id(&x.borrow().get_name()))
                                .collect();
                            method_vertices.insert(name.clone(), subtasks.clone());
                            for elem in subtasks.iter() {
                                working_set.push_back(*elem);
                                if !task_vertices.contains_key(elem)
                                    && domain.get_task(*elem).borrow().is_primitive()
                                {
...
mod tests {
    ...
    use super::*;
    use crate::{
        domain_description::{DomainTasks, FONDProblem, Facts},
        task_network::{CompoundTask, HTN, Method, PrimitiveAction, Task},
    };
    #[test]
    pub fn tdg_correctnes_test() {
        ...
        let domain = domain.add_methods(vec![
            (domain.get_id("t1"), t1_m),
            (domain.get_id("t2"), t2_m),
            (domain.get_id("t3"), t3_m),
            (domain.get_id("t4"), t4_m),
            (domain.get_id("unreach_t"), unreach_m),
        ]);
        let init_tn = HTN::new(
            BTreeSet::from([1, 2, 3]),
            vec![(1, 3), (2, 3)],
    ...
    pub fn tn_reachability_test() {
        ...
        let tdg = TDG::new(&tn);
        if let Task::Compound(CompoundTask { name: _, methods }) = &*tn.get_all_tasks()[0].borrow() {
            let new_tn = tn.decompose(*tn.get_nodes().iter().next().unwrap(), &methods[0]);
            let p1_id = new_tn
                .get_all_tasks_with_ids()
                .iter()
                .filter(|(x, _)| x.borrow().get_name() == "p1")
                .collect::<Vec<_>>()[0]
                .1;
    ...
    pub fn cyclic_task_test() {
        ...
        assert_eq!(
            domain
                .get_task(reachables.pop_last().unwrap())
                .borrow()
                .get_name(),
            format!("t1")
        );
