use super::{CompoundTask, PrimitiveAction, Task, HTN};
use crate::{
    domain_description::{DomainTasks, FONDProblem},
    task_network::Method,
};
use std::{
    cell::RefCell,
...
impl OutcomeDeterminizer {
    ...
    pub fn from_fond_problem(problem: &FONDProblem) -> (FONDProblem, HashMap<u32, u32>) {
        ...
        // substitue nd tasks
        let (new_tasks, bijection) =
            OutcomeDeterminizer::substitue_nd_tasks(all_tasks, &nd_actions);
        let new_domain = DomainTasks::new(new_tasks);
        let nd_act_map: Vec<(u32, Vec<u32>)> = nd_actions
            .iter()
            .map(|(_, (c, det_acts))| {
                (
                    ...
                    det_acts
                        .iter()
                        .map(|x| new_domain.get_id(&x.get_name()))
                        .collect::<Vec<_>>(),
                )
            })
            .collect();
    ...
    fn to_abstract(action: &PrimitiveAction) -> (CompoundTask, Vec<PrimitiveAction>) {
        ...
        let c = CompoundTask {
            name: action.name.clone() + "__determinized",
            methods: vec![],
        };
            ...
            match &*task.borrow() {
                ...
                Task::Primitive(action) => {
                    if !action.is_deterministic() {
                        ...
                        let det_actions: Vec<Task> = det_actions
                            .into_iter()
    ...
    fn substitue_nd_tasks(
        all_tasks: &Vec<RefCell<Task>>,
        nd_actions: &HashMap<usize, (Task, Vec<Task>)>,
    ) -> (Vec<Task>, HashMap<u32, u32>) {
        let mut new_tasks: Vec<Task> = all_tasks
            .iter()
            .enumerate()
            .filter(|(i, _t)| !nd_actions.contains_key(i))
            .map(|(_i, x)| x.borrow().clone())
            .collect();
        ...
        for t in new_tasks.iter_mut() {
            if let Task::Compound(CompoundTask { name: _, methods }) = t {
                for m in methods.iter_mut() {
                    for (_node_id, task_id) in m.decomposition.mappings.iter_mut() {
    ...
    fn compute_bijection(
        ...
        for (prev_id, prev_task) in all_tasks.iter().enumerate() {
            ...
            let new_name = match &*borrowed_task {
                Task::Compound(_) => borrowed_task.get_name(),
                Task::Primitive(p) => {
            ...
            let new_id = new_tasks
                .iter()
                .position(|x| x.get_name() == new_name)
                .unwrap();
...
mod tests {
    ...
    pub fn determinization_test() {
        ...
        assert_eq!(
            *bijection.get(&domain.get_id("p1")).unwrap(),
            relaxed.tasks.get_id("p1")
        );
        assert_eq!(
            *bijection.get(&domain.get_id("p2")).unwrap(),
            relaxed.tasks.get_id("p2__determinized")
        );
        assert_eq!(
            *bijection.get(&domain.get_id("p3")).unwrap(),
            relaxed.tasks.get_id("p3")
        );
        assert_eq!(
            *bijection.get(&domain.get_id("t1")).unwrap(),
            relaxed.tasks.get_id("t1")
        );
        assert_eq!(relaxed.facts.count(), problem.facts.count());
        let new_tasks: Vec<String> = relaxed
            .tasks
            .get_all_tasks()
            .iter()
        ...
        if let Task::Compound(CompoundTask { name: _, methods }) =
            &*relaxed.tasks.get_task(id_p2).borrow()
        {
            let mut det_acts = vec![
                relaxed.tasks.get_id("p2__determinized_0"),
                relaxed.tasks.get_id("p2__determinized_1"),
            ];
        ...
        if let Task::Compound(t) = &*relaxed.init_tn.get_all_tasks()[0].borrow() {
            ...
            let task_names: Vec<String> = decomp
                .get_all_tasks()
                .iter()
                .map(|x| x.borrow().get_name())
                .collect();
